...
// A helper function for reading JHI path from registry keys
...
*/
FARPROC WINAPI delayHook(unsigned dliNotify, 
						PDelayLoadInfo pdli)
{
	switch (dliNotify) {
		...
		// This is the event we are interested in - a DLL needs to be loaded
		case dliNotePreLoadLibrary :
			{
				// Get the name of the DLL that the application is trying to delay load
				...
				wchar_t dllName[9]; 
				size_t outSize;
				mbstowcs_s(&outSize, dllName, 9, "\\JHI.dll", 8);
				dllName[8] = '\0';
				...
				else
				{
					...
					delete[] wtRegValue;
					throw std::exception("JHI.dll could not be located.\nTo run over Intel(R) DAL simulator, ensure Amulet configuration is selected and Emulauncher is running properly.");					
				}
				...
				if(!VerifyFileSignature(path) || !VerifyCertificateName(path))
				{
					// JHI dll is not signed by Intel
					delete[] wtRegValue;
					throw std::exception("JHI.dll signature could not be verified.");
				}
				
				cout << "JHI.dll verification was successful. Loading JHI.dll." << endl;
#endif
