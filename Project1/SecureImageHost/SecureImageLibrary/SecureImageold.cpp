...
SecureImage::SecureImage(void)
{
	try
	{
		...
		//taPath = "\\OurProject\\Project1\\SecureImageApplet\\bin\\SecureImageApplet.dalp";
		// added the line below instead of the line above:
		...
		else
		{
			...
			ret=JHI_Install2(handle,taId.c_str(),ws.c_str());	// this is the point where it falls
			if(ret != JHI_SUCCESS)
			{
