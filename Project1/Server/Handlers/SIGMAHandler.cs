...
namespace DALSamplesServer
{
    class SIGMAHandler
    {
        ...
        byte[] Ga = new byte[DataStructs.SIGMA_PUBLIC_KEY_LEN];//The prover's ephemeral DH public key
        byte[] Gb = new byte[DataStructs.SIGMA_PUBLIC_KEY_LEN];//The verfier's ephemeral DH public key.
        byte[] _gagb = new byte[DataStructs.SIGMA_PUBLIC_KEY_LEN * 2];//property
        byte[] Sk = new byte[DataStructs.SIGMA_SESSION_KEY_LEN];//Session Confidentiality Key: 128 bit key derived from SMK
        byte[] Mk = new byte[DataStructs.SIGMA_MAC_KEY_LEN];//Session Integrity Key: 128bit key derived from SMK.  
        ...
        public const string OCSP_CER_FILE = "ocspFile.cer";
        public const string OCSP_CER_FROM_CHAIN_FILE = "certFromOcspChain.cer";
        public const string OCSP_SERVER_NAME = "http://upgrades.intel.com/ocsp";
        ...
        public static List<byte[]> SplitCertificateChain(byte[] origCert)
        {
        ...
        byte[] GenerateX509VerifierCert(byte[] cert)
        {
